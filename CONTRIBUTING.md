...
**Quick build** (uses default version `1.9.8`) â€” double-click `installer\Build-Installer.cmd`

**Versioned build:**

```powershell
powershell -ExecutionPolicy Bypass -File .\installer\Build-Installer.ps1 -Version 1.9.8
```

**Output:**

```text
dist\SS-CAM-v1.9.8.exe   (~48 MB)
```
...
```powershell
.\dist\SS-CAM-v1.9.8.exe --smoke-test
```
...
# Commit
git commit -m "feat(release): publish SS-CAM v1.9.8"

# Tag the release
git tag -a v1.9.8 -m "SuamiSihat Creative Assets Management v1.9.8"
