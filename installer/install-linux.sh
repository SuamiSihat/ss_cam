...
main() {
    ...
    local FALLBACK_TAR_URL="${RELEASE_BASE}/ss-cam-linux-x64.tar.gz"
    local ICON_URL="https://raw.githubusercontent.com/SuamiSihat/ss_cam/SS-Master/installer/assets/ss-cam.svg"
    local CURL_INSTALL_URL="https://suamisihat.github.io/ss-cam/install.sh"
    ...
    if [ $INSTALLED -eq 0 ]; then
        ...
        if [ -f "$LOCAL_TAR" ]; then
            ...
            if curl -fL --progress-bar --connect-timeout 20 --retry 2 -o "$TEMP_TAR" "$RELEASE_TAR_URL" 2>/dev/null; then
                ...
                stop_spinner
                echo -e "  ${C_GREEN}✔ Downloaded (fallback URL).${C_RESET}"
            else
                stop_spinner
